...
for instance in tqdm(range(instances_number)):
    ...
    params_bounds = {'gamma': (0.2, 0.6), 'tau': (2, 20)}
    ...
    #
    sgap_df = qmcmc_runner.run_random_qmcmc(mc_steps, sgap_df, params_bounds=params_bounds)
#
# print(sgap_df)
...
sg_mean = numpy.full(shape=5, fill_value=numpy.mean(numpy.array(sgap_df['sgap mean'])))
sg_std = numpy.full(shape=5, fill_value=numpy.mean(numpy.array(sgap_df['sgap std'])))
# printing plots
